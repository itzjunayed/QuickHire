...
export default function SignupPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (user) {
        // User is already logged in, redirect to home or jobs
        router.push('/jobs');
      }
    }
  }, [router]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-primary-50">
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 py-16 sm:py-24 relative z-10">
        {/* Card */}
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8">
          {/* Header */}
          <h1 className="text-3xl font-bold text-dark mb-2">Get Started</h1>
          <p className="text-gray-500 text-sm mb-8">Create your QuickHire account in minutes</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-primary hover:bg-primary-600 disabled:bg-gray-400 text-white font-semibold py-3 rounded-lg transition-colors mt-6"
            >
